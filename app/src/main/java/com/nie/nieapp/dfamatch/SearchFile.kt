...
class SearchFile {
    ...
    fun searchFileForExtension(extensions: List<String>, file: File = Environment.getExternalStorageDirectory()): ArrayList<File> {
        return file
                .walk()
                .maxDepth(Int.MAX_VALUE)
                .filter { it.isFile && extensions.contains(it.extension) }
                .mapTo(ArrayList()) { it }
    }
    ...
    fun searchVfForHead(file: File = Environment.getExternalStorageDirectory()): ArrayList<File> {
        val videKeyMap = DfaMatch.createSensitiveMapBytes(videoHeadList)
    ...
    companion object {
        ...
        val videoHeadList = arrayListOf(t_rmvb, t_mpeg, t_wmf_1, t_wmf_2, t_3gp_1, t_3gp_2, t_3gp_3, t_mp4_1, t_mp4_2, t_avi)
        //video后缀
        val videoExtensions = arrayListOf("mp4", "avi", "mpeg", "mov", "asf", "wmf", "rm", "rmvb", "flv", "swf")

        val apkExtensions = arrayListOf("apk")
    }
}